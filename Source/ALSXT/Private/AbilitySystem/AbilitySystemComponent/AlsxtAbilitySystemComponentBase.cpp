...
void UAlsxtAbilitySystemComponentBase::InitAbilityActorInfo(AActor* InOwnerActor, AActor* InAvatarActor)
{
	Super::InitAbilityActorInfo(InOwnerActor, InAvatarActor);
}

void UAlsxtAbilitySystemComponentBase::OnGameplayEffectAddedCallback(UAbilitySystemComponent* const ASC, const FGameplayEffectSpec& Spec, FActiveGameplayEffectHandle Handle)
{
...
void UAlsxtAbilitySystemComponentBase::OnGameplayEffectRemovedCallback(const FActiveGameplayEffect& ActiveGameplayEffect)
{
...
const UAttributeSet* UAlsxtAbilitySystemComponentBase::GetOrCreateAttributeSet(const TSubclassOf<UAttributeSet>& InAttributeSet)
{
	return GetOrCreateAttributeSubobject(InAttributeSet);
}

void UAlsxtAbilitySystemComponentBase::ChangeLevel_Implementation(const float DesiredLevel, const bool bInitialization /* = false */)
{
...
FActiveGameplayEffectHandle UAlsxtAbilitySystemComponentBase::SetActiveGameplayEffectInhibit(FActiveGameplayEffectHandle&& ActiveGEHandle, bool bInhibit, bool bInvokeGameplayCueEvents)
{
...
float UAlsxtAbilitySystemComponentBase::GetFilteredAttribute(const FGameplayAttribute Attribute, const FGameplayTagRequirements SourceTags, const FGameplayTagContainer TargetTags)
{
	return GetFilteredAttributeValue(Attribute, SourceTags, TargetTags);
}

FActiveGameplayEffectHandle UAlsxtAbilitySystemComponentBase::ApplyGameplayEffectSpecToTarget(const FGameplayEffectSpec& GameplayEffect, UAbilitySystemComponent* Target, FPredictionKey PredictionKey)
{
	if (Target->GetOwner())
	{
 ...
 void UAlsxtAbilitySystemComponentBase::InitializeAbilitySystemData(const TSoftObjectPtr<UAlsxtAbilitySystemInitializationDataAsset> InitializationData, AActor* InOwningActor, AActor* InAvatarActor)
 {
	...
	if (AbilitySystemDataInitialized)
 	 ...
 	 */
 	OnActiveGameplayEffectAddedDelegateToSelf.AddUObject(this, &UAlsxtAbilitySystemComponentBase::OnGameplayEffectAddedCallback);
 	ActiveGameplayEffects.OnActiveGameplayEffectRemovedDelegate.AddUObject(this, &UAlsxtAbilitySystemComponentBase::OnGameplayEffectRemovedCallback);
